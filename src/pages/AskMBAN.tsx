...
function runSearch(
  ...
  for (const c of courses) {
    ...
    // In Learn mode skip instructors
  ...
  for (const p of projects) {
    ...
    const methodScore  = p.methods.reduce((a, m) => a + scoreText(m, q) * 2, 0)
    const toolScore    = p.tools.reduce((a, t) => a + scoreText(t, q), 0)
    const kwScore      = p.search_keywords.reduce((a, k) => a + scoreText(k, q) * 2, 0)
    // In Learn mode skip professor field
    const profScore = mode === 'find' ? scoreText(p.professor, q) * 3 : 0
    const base = nameScore + probScore + taughtScore + courseScore + methodScore + toolScore + kwScore + profScore
    if (base <= 0) continue
...
function findPrimaryAnswer(rawQuery: string, results: SearchResult[]): PrimaryAnswer {
  ...
  const topMethod = results.find(
    r => r.kind === 'method' && r.score >= 8
  ) as Extract<SearchResult, { kind: 'method' }> | undefined
...
export default function AskMBAN() {
  ...
  return (
    <div className={`p-5 md:p-10 max-w-3xl mx-auto ${hideChips ? 'pt-3' : ''}`}>
      ...
      {/* Header — hide on mobile focus */}
      {!hideChips && (
        <div className="mb-8 text-center">
          <h1 className="text-2xl md:text-3xl font-bold text-white mb-2">Ask MBAN</h1>
          <p className="text-gray-400 text-sm">
            Ask a study question, compare methods, or review a course concept.
          </p>
          <p className="text-gray-600 text-xs mt-1">
            {methods.length} methods · {courses.length} courses · {projects.length} projects · {totalSlides} slides
          </p>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="mb-6">
        <div className="relative">
          <input
            ...
            placeholder="Ask a study question: explain overfitting, compare k-fold vs bootstrap, or review LP formulation"
        ...
        <div className="mb-10">
          ...
          {results.length === 0 ? (
            <div className="bg-gray-800 border border-gray-700 rounded-2xl p-10 text-center">
              ...
              <p className="text-gray-600 text-xs mt-1">
                Try a concept, method comparison, course code, or assignment topic
              </p>
            </div>
          ) : (
